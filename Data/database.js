...
function processData(carin, carout) {
  ...
  const query = "INSERT INTO public.project (time, carin, carout, total) VALUES (localtimestamp, $1, $2, $3)";
...
function sendDataAPI() {
    return new Promise((resolve, reject) => {
        Promise.all([
            ...
            `),
            db.one(`
                SELECT COALESCE(SUM(p.carin), 0) AS carin
                FROM project AS p
            `),
            db.one(`
                SELECT COALESCE(SUM(p.carout), 0) AS carout
                FROM project AS p
            `)
        ])
        .then(([data1, result, data2, data3]) => {
            ...
            resolve({ data1, totalcar: totalRemainingCars, carinall: data2.carin, caroutall: data3.carout }); // Resolve data
        })
        .catch((error) => {
...
module.exports = { processData, sendDataAPI};
